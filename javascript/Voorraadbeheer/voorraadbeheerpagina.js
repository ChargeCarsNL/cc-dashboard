...
async function getArtikelen() {
  ...
  try {
    const response = await proxyFetch(url, {
      ...
      body: {
        // Filter artikelen op artikelen waar voorraad bij wordt gehouden (Voorraad item = true)
        filter: {
          operator: "and",
          fields: [
            {
              comparison: "is",
              field: "s95756d359", // voorraad item (true / false veld slug)
              value: true,
            },
          ],
        },
    ...
    console.log("artikelen response:", response); // Log the response

    const data = await response;
    const artikelenArray = data.items;
...
function intializePage() {
  const url =
    "https://prod-188.westeurope.logic.azure.com:443/workflows/c692d1e2c0494c1fad8e6eb43777380f/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=GLLeWlwV-sS7pgnipNEhZyJOEkodz534uhuDmzkbXfY";
...
async function getUnitClass(scannedCode) {
  ...
  for (let i = 0; i < unitClassItemsArray.length; i++) {
    ...
    let EANCode = getCustomFieldValueById(
      customFieldsArray,
      EANCodeCustomFieldId
    );
...
function addUnitToSelectedVoorraad(
  ...
  console.log(
    `Unit is bekend: ${currentUnitClassItem.name}. Unit wordt toegevoegd aan ${selectedVoorraad.name}...`
  );
  ...
  const url =
    "https://prod-189.westeurope.logic.azure.com:443/workflows/db5e25179b4b4ac8bb43551fb17388ba/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=e68NLwrVMujBi3KiJ7SvEkGUMWDAL8Vukh8Qx88QD38";
  console.log(
    `item: ${unitClassName} - ${scannedCode} wordt aan voorraad lijst toegevoegd`
  );
  fetch(url, {
    method: "POST",
    headers: {
    ...
    .then((data) => {
      ...
      barcodeInputElement.value = "";
      runSuccesMessage(
        `<strong>${unitClassName}</strong> succesvol toegevoegd aan ${selectedVoorraad.name}`
      );
      focusBarcodeInput();
    })
    .catch((error) => {
      ...
      barcodeInputElement.value = "";
      runErrorMessage(
        `<strong>${unitClassName}</strong> kon niet worden toegevoegd aan ${selectedVoorraad.name}`
      );
      focusBarcodeInput();
    });
}
